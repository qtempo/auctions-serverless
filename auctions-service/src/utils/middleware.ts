...
interface LambdaHandler<B, P, Q> extends Handler<APIGatewayProxyEvent, APIGatewayProxyResult> {
  handler: (handler: AuctionsLambdaHandler<B, P, Q>) => LambdaHandler<B, P, Q>
}
...
export const middleware = <B, P, Q>() => {
  ...
  const middlewareHandler: LambdaHandler<B, P, Q> = async (event, context) => {
    try {
